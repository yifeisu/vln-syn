...
class Seq2SeqAgent(BaseAgent):
    ...
    def make_equiv_action(self, a_t, perm_obs, perm_idx=None, traj=None):
        ...
        def take_action(idx, name):
            ...
            # else:
            #     # Adjust
            #     self.env.env.sims[idx].makeAction(*[[item] for item in self.env_actions[name]])

            # simulator v2
            batch_action = [[0] * self.env.batch_size, [0] * self.env.batch_size, [0] * self.env.batch_size]
            if type(name) is int:
                # Go to the next view
                batch_action[0][idx] = name
            else:
                # Ajsut the heading
                batch_action[0][idx], batch_action[1][idx], batch_action[2][idx] = self.env_actions[name]

            self.env.env.sim.makeAction(*batch_action)
        ...
        for i, idx in enumerate(perm_idx):
            ...
            if action != -1:  # -1 is the <stop> action
                ...
                while src_level > trg_level:  # Tune down
                    ...
                    src_level -= 1
                # Turn right until see the target, then move to the target candidate
                while self.env.env.sim.getState()[idx].viewIndex != trg_point:
                    take_action(idx, 'right')
                assert select_candidate['viewpointId'] == \
                       self.env.env.sim.getState()[idx].navigableLocations[select_candidate['idx']].viewpointId
                take_action(idx, select_candidate['idx'])

                state = self.env.env.sim.getState()[idx]
                if traj is not None:
