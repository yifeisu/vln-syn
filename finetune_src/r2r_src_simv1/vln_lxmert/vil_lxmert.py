...
class LXRTXLayer(nn.Module):
    ...
    def state_x_lang_vis(self, state, lang_visual_input, lang_visual_attention_mask):
        # Cross Attention
        state_att_output = self.visual_attention(state,
                                                 lang_visual_input,
                                                 ctx_att_mask=lang_visual_attention_mask)
        return state_att_output
    ...
    def forward(self, lang_feats, lang_attention_mask, visual_feats, visual_attention_mask):
        # # 1. vision information attend state-language, update the vision information.
        # visual_att_outputs = self.cross_att(lang_feats,
        #                                     lang_attention_mask,
        #                                     visual_feats,
        #                                     visual_attention_mask)
        # visual_att_output = self.self_att(visual_att_outputs[0], visual_attention_mask)
        # visual_output = self.output_fc(visual_att_output[0])
        #
        # # 2. state attend the language-vision information, update the state.
        # state_t = lang_feats[:, 0:1, :]  # batch x 1 x hidden_size.
        # lang_visual = torch.cat([lang_feats[:, 1:, :], visual_output], dim=1)
        # lang_visual_attention_mask = torch.cat([lang_attention_mask[:, :, :, 1:], visual_attention_mask], dim=-1)
        # state_att_outputs = self.state_x_lang_vis(state_t, lang_visual, lang_visual_attention_mask)
        #
        # # 3. get the attention scores to language and vision.
        # lang_len, vis_len = lang_feats.shape[1] - 1, visual_output.shape[1]
        # state_att_probs = state_att_outputs[1]
        # assert (lang_len + vis_len) == state_att_probs.shape[-1]
        #
        # lang_att_scores = state_att_probs[:, :, 0, :lang_len]  # batch, num_head, target_len
        # visual_att_scores = state_att_probs[:, :, 0, lang_len:]  # batch, num_head, target_len
        #
        # # 4. cat the updated state and language for the next layer
        # lang_output = torch.cat([state_att_outputs[0], lang_feats[:, 1:, :]], dim=1)

        visn_att_output = torch.cat((lang_feats[:, 0:1, :], visual_feats), dim=1)
        state_vis_mask = torch.cat((lang_attention_mask[:, :, :, 0:1], visual_attention_mask), dim=-1)
        ...
        lang_att_scores = cross_attention_scores[:, :, 0, :]
        ...
        visual_output = state_visn_output[:, 1:, :]
        lang_output = torch.cat((state_visn_output[:, 0:1, :], lang_feats[:, 1:, :]), dim=1)

        visual_att_scores = state_visn_att_output[1][:, :, 0, 1:]

        return lang_output, visual_output, lang_att_scores, visual_att_scores
...
class LxmertEncoder(nn.Module):
    ...
    def forward(self,
        ...
        if mode == 'language':
            ...
            output_img_embedding = self.visn_fc(visual_feats)
            text_embeds = lang_feats

            # Run cross-modality layers
            lang_output, visual_output = text_embeds, output_img_embedding
            for layer_module in self.x_layers:
                x_outputs = layer_module(lang_output,
                                         lang_attention_mask,
                                         visual_output,
                                         visual_attention_mask)

                lang_output, visual_output, lang_att_scores, visual_att_scores = x_outputs

            # get the attended language and image features
            language_state_scores = lang_att_scores.mean(dim=1)
            visual_action_scores = visual_att_scores.mean(dim=1)
...
class VlnModel(LxmertPreTrainedModel):
    ...
    def forward(self,
        ...
        if mode == 'language':
            ...
            # Run Lxmert encoder
            encoder_outputs = self.encoder(mode,
                                           embedding_output,
